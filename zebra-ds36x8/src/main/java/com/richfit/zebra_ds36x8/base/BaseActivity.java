...
import com.richfit.zebra_ds36x8.activity.DeviceListActivity;
...
import com.richfit.zebra_ds36x8.utils.ToastUtil;
...
import static com.richfit.zebra_ds36x8.helpers.Constants.DEBUG_TYPE.TYPE_DEBUG;
import static com.richfit.zebra_ds36x8.helpers.Constants.logAsMessage;
import static com.zebra.scannercontrol.DCSSDKDefs.DCSSDK_CONN_TYPES.DCSSDK_CONNTYPE_BT_NORMAL;

public abstract class BaseActivity extends AppCompatActivity implements ScannerAppEngine, IDcsSdkApiDelegate {
    ...
    public static boolean waitingForFWReboot = false;
    ...
    public DCSSDKDefs.DCSSDK_RESULT connect(int scannerId) {
        if (Application.sdkHandler != null) {
            if (DeviceListActivity.curAvailableScanner != null) {
                Application.sdkHandler.dcssdkTerminateCommunicationSession(DeviceListActivity.curAvailableScanner.getScannerId());
            }
    ...
    public void disconnect(int scannerId) {
        if (Application.sdkHandler != null) {
            ...
            DeviceListActivity.curAvailableScanner = null;
            updateScannersList();
        }
    }
                ...
                case Constants.BARCODE_RECEIVED:
                    ...
                    if (Application.MOT_SETTING_NOTIFICATION_BARCODE && !notificaton_processed) {
                        ...
                        if (isInBackgroundMode(getApplicationContext())) {
                            ...
                            ToastUtil.shortToast(getApplicationContext(), "Barcode received from " + scannerName);
                        }
                    }
//                    if (!Application.isFirmwareUpdateInProgress) {
//                        if (Application.CurScannerId != Application.SCANNER_ID_NONE) {
//                            Intent intent = new Intent("activity.ScannersSimpleActivity");
//                            intent.setComponent(new ComponentName(getPackageName(), ScannersActivity.class.getName()));
...
//                    }
                ...
                case Constants.SESSION_ESTABLISHED:
                    logAsMessage(TYPE_DEBUG, TAG, " do SESSION_ESTABLISHED ");
                    DCSScannerInfo activeScanner = (DCSScannerInfo) msg.obj;
                    notificaton_processed = false;
                    result = false;
                    DeviceListActivity.curAvailableScanner = new AvailableScanner(activeScanner);
                    DeviceListActivity.curAvailableScanner.setConnected(true);
                    ...
                    if (Application.MOT_SETTING_NOTIFICATION_ACTIVE && !notificaton_processed) {
                        ...
                        if (!found) {
                            notification_Msg.append(activeScanner.getScannerName() + " has appeared and connected");
                        } else {
                            notification_Msg.append(activeScanner.getScannerName() + " has connected");
                        }
                        if (isInBackgroundMode(getApplicationContext())) {
                            Intent intent = new Intent();
                            ...
                            ToastUtil.shortToast(getApplicationContext(), notification_Msg.toString());
                        }
                    }
                    break;
                case Constants.SESSION_TERMINATED:
                    logAsMessage(TYPE_DEBUG, TAG, " do SESSION_TERMINATED ");
                    ...
                    if (Application.MOT_SETTING_NOTIFICATION_ACTIVE && !notificaton_processed) {
                        if (isInBackgroundMode(getApplicationContext())) {
                            ...
                            ToastUtil.shortToast(getApplicationContext(), scannerName + " has disconnected");
                        }
                    }
                    break;
                case Constants.SCANNER_APPEARED:
                case Constants.AUX_SCANNER_CONNECTED:
                    logAsMessage(TYPE_DEBUG, TAG, " do SCANNER_APPEARED & AUX_SCANNER_CONNECTED ");
                    notificaton_processed = false;
                    result = false;
                    ...
                    if (Application.MOT_SETTING_NOTIFICATION_AVAILABLE && !notificaton_processed) {
                        if (isInBackgroundMode(getApplicationContext())) {
                            ...
                            ToastUtil.shortToast(getApplicationContext(), availableScanner.getScannerName() + " has appeared");
                        }
                    }
                    break;
                case Constants.SCANNER_DISAPPEARED:
                    logAsMessage(TYPE_DEBUG, TAG, " do SCANNER_DISAPPEARED ");
                    notificaton_processed = false;
                    result = false;
                    ...
                    if (Application.MOT_SETTING_NOTIFICATION_AVAILABLE && !notificaton_processed) {
                        StringBuilder notification_Msg = new StringBuilder();
                        notification_Msg.append(scannerName + " has disappeared");
                        if (isInBackgroundMode(getApplicationContext())) {
                            Intent intent = new Intent();
                            intent.setAction(Constants.ACTION_SCANNER_CONNECTED);
                            intent.putExtra(Constants.NOTIFICATIONS_TEXT, notification_Msg.toString());
                    ...
                    if ((DeviceListActivity.curAvailableScanner != null) && (scannerID == DeviceListActivity.curAvailableScanner.getScannerId())) {
                        DeviceListActivity.curAvailableScanner = null;
                    }
                    break;
            }
